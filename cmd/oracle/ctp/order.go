...
type OrderState struct {
	...
	ErrorID int
	// OrderSysID 是交易所委托号，取自 OnRtnOrder；**空表示回报里从没出现过**。
	//
	// ⚠️ 20260915 评审建议补的：§13 #6 有一个分不开的替代解释 —— 价格类拒单是**柜台前置**拒的
	// 还是**交易所**拒的。非空 ⇒ 这笔进过交易所；空 ⇒ 没有交易所编号，
	// 而「柜台前置拒」与「交易所在分配编号之前拒」**这一个字段分不开**。
	OrderSysID string
}
