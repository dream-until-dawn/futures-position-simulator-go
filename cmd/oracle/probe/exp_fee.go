...
//
// ⚠️ 这个问题的提法是被一次**错误结论**逼出来的，原文留在这里当反例：
// 先量了 rb2701 / rb2610 / m2701，保证金率反解全部精确等于 7%，
// 于是我写下「快期用的是统一简化值，不能当费率的对拍口子」。
// **那是错的**——那三个合约恰好全落在同一档费率上，样本没有判别力；
// 换上 i / cu 立刻出现 11%，ag 是 22%。快期加载的是逐品种的真实费率。
// 所以本实验真正要问的是「跨品种到底一不一样」，而不是去确认一个已有的猜想。
//
...
//
// ⚠️ 基准取昨结算价，是因为它在所有已测合约上都给出精确的整百分数
// （7% / 11% / 22%），而最新价基准给出一批互不相同的脏数。
//
// ⚠️ 两个方向的结论强度**不对称**，别对称地读这份输出：
// 「各品种的率不同」是一条**证伪**，直接推翻「全局统一」；
// 「本批的率全部相同」什么也证明不了——它同样可能只是样本落在了同一档上。
// 这一条是踩出来的，见本文件开头。
