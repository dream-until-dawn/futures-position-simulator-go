...
//
// ⚠️⚠️ **20260910 夜盘更正：上面那句「（CTP 没有）↔ OpenTodayPreSettleHistory」
// 与下面这句「SimNow 实际配的是 OpenPrice，而本库表达不了它」——**都是错的**。
//
// 原来的读法只看了柜台的**声明**（`MarginPriceType "4" 开仓价`）。
// 而把今仓与昨仓的**行为**一起量下来，得到的正是本枚举：
//
//	今仓  开仓价      实测两次：3146 → 5033.60、3148 → 5036.80
//	昨仓  昨结算价    实测一次：3147 → 5035.20（事前登记的三个候选里只有它命中）
//	⇒ 合起来 = **OpenTodayPreSettleHistory**
//
// ⚠️ `MarginPriceType "4"` 没说错，它管的是**今仓**；结算把昨仓重置到了昨结算价
// （`PositionCost` 31480 → 31470，而 `OpenCost` 不动）。
// ⇒ **错的是把一句关于今仓的声明读成「对所有持仓都成立」。**
//
//	⚠️ 一个「本库表达不了柜台」的结论，来自只读了柜台的自述。
//	而那个结论会让人去改本库 —— 改掉一个本来就对的模型。
//
// 证据与事前登记见 state.md「判别实验 ① 有答案了」；
// 对拍见 conformance/ctpfixture（昨仓样本 1 条，残差 1e-12 = CTP double 的噪声）。
//
