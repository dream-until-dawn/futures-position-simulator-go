...
//
// 门面（本包的 Simulator）落地了前两期：灌成交路径（New、出入金、Mark、ApplyTrade、查询）与日终结算（Settle）。
// ⚠️ 报单路径、挂单冻结、状态存取还没有；口径（Choices）六项全部由调用方显式选，零值报错。
//
// ⚠️ `go get` 在没有 tag 时默认拿到 `main`，而 `main` 可能落后于 `dev`。
