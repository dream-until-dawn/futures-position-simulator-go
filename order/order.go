...
//
// ⚠️ 边界要一起记住：两个合约、两家交易所、一个口子；而**判出先后的只有
// 几对，其余的顺序是猜的** —— 猜的部分与量出来的部分在代码上长得一模一样。
// 具体哪几对判出来了，见 state.md 的 `reject_priority_measured`。
// ⚠️ 这里**刻意不写数字**：那个数当天从 3 涨到 4，而五处复述里只有一处跟上了。
package order
