...
- **涨跌停取整**：`MeasuredTickRounding()` 只给实测过的两家 —— 上期所向下（probes.md §12）、大商所往里收（§13 #24 有条件收敛，前提是比例为整百分比；F12 之前是四舍五入）。能源中心 / 郑商所 / 广期所 / 中金所**不填**
...
##### 决策点（F7c 之前定；F7c 评审 20260915 同意，合进 main 483396d / 272ec9e）
...
##### 形状（评审 20260916 同意，合进 main 4560064；落地见下「F8 落地」）
...
✅ **使用者 20260922 裁决（双方各自确认：实现方会话里答「1 同意 / 3 同意」，评审会话里逐条问过、都答「是，同意」）**：
决策点 1 按倾向 —— 新增第九项口径 `Choices.RestingFill`（触价即成 / 穿价才成），零值在 F9c 撮合入口报错，两个预设都不给值；
决策点 3 按倾向 —— 按挂单先后，F9a 给委托簿记序号、`State` / `Restore` 带上、**抬 `StateFormat` 一格**（旧存档在版本号那一步报错，不迁移）。
✅ **P-resting-price：使用者 20260924 定了「节后跑（9/28 起）」**（评审会话里用 AskUserQuestion 向使用者本人核的，实现方会话里同日转达并记录）。具体哪一天由实现方排。
⚠️ 开跑前**方案单独送审一次**，不与实现批混在一起：挂哪个合约、几手、挂多久、被触到的判据、没触到怎么收工、挂单期间的敞口怎么控 —— 这是 F9 三项里风险最高的一个（真实挂单等行情来触）。
...
##### 形状（初版：评审 20260918 同意，合进 main ba43a12 —— ⚠️ **已被本节「20260921 改写」取代**（设计合并 1ce8960、实现合并 2bad027），约束以改写为准：每笔 `|atSettle − fee| ≤ 0.01` **且 `atSettle ≥ 0`**，不是这里的 `0 ≤ atSettle ≤ fee`）
...
2. 三条**推得、未实测**的外推：**一律「报错不猜」**，与本库既有做法一致（#21 往郑商所推时使用者定「其余报错不猜」、郑商所 X0 的静默错修成整笔报错、`chargeUndated` 注释「没有可用的规则 —— 不猜」）。
   ✅ **20260922 F13（§17）**：交易日 20260923 两所各一次事前登记实验 ⇒ **多手裸平放开为按额度拆、额度放开为分方向**；「显式平今扣不扣额度」照旧报错（显式平今在两所上被柜台改写成通用平仓，§13 #25）。下面是 F11 当时的原文。
...
- **三条外推的报错**（⚠️ F13 之后只剩「显式平今之后」那一条，见 §17）：多手裸平且 0 < 额度 < 平仓量 ⇒ 报错、额度 0 或足够 ⇒ 照收；显式平今之后的裸平两种读法分岔 ⇒ 报错、同值 ⇒ 照收；当日开过反方向且两种读法分岔 ⇒ 报错。每一格报错后状态不动
- **挂单**：冻结时按当时额度、成交时按成交时额度；两笔裸平挂单都按同一个额度冻、先成交的那笔用掉额度后第二笔按剩余收

##### ⚠️ 风险与盲区（实现之前先写下）

- 只有一个柜台，两个交易所各一次三笔（§13 #23 不进 `rules_measured`）
- 上面三条外推都选了**报错**（⚠️ F13 放开了多手与分方向两条，§17）：代价是调用方在那几种形状上拿不到数（多手裸平、显式平今之后的裸平、当日开过反方向的仓）。报错信息要点名是哪一条、哪个 §13 条目，调用方能拆成一手一手报或改用显式标志绕开；每条排一个事前登记的实验，测到了再放开
...
#### 17. F13：F11 两条外推放开 —— 多手裸平按额度拆、额度分方向（2026-09-22，实现之前写）

§15 决策点 2 给三条没实测的外推选了「读法分岔 ⇒ 报错」，每条排一个事前登记的实验。交易日 20260923 夜盘两所各跑了一次（cn-futures-rules.md §13 #23 行尾；原始记录在 state.md 两个「事前登记：F11 …外推」登记块）：

    A 多手裸平、0 < 额度 < 平仓量   两所都只剩「按额度拆」：一条 2 手的成交记录，m2703 收 0.3、MA703 收 8（= 1 手平今 + 1 手平昨）
    C 当日开过反方向               两所都只剩「分方向」：m2705 收 0.2、MA705 收 2（多头额度 0 ⇒ 平昨档）
    B 显式平今扣不扣额度           两所都不判：显式平今被柜台改写成通用平仓（§13 #25，观测、未收敛；silent-risks 103）

⇒ 本批只放开 **A 与 C**；B 那一格照旧报错，silent-risks 103（本库在 `Submit` / `Place` 路径上按标志处理显式平今）等 #25 的事前登记复现之后另开一批。

##### 已核（20260922 夜，读代码与语料）

| 事实 | 出处 | 对设计的影响 |
|---|---|---|
| 三条外推的判定全在一个纯函数里：四种读法（分 / 不分方向 × 扣 / 不扣显式平今），任一种落在 (0, 平仓量) 或两种给出不同手数就报错 | `quota.go` `todayTierLots` | 改动集中在这一处：读法只留「本方向 × 扣 / 不扣」两种，额度落在 (0, 平仓量) 时取 min，不报错 |
| 钉住三条报错的测试只有两条：`TestTodayTierLots`（纯函数逐格）与 `TestQuotaRefusalsLeaveNoTrace`（门面上报错后状态不动）；`conformance/**` 里没有一条走到这几种形状 | `quota_test.go` | 这两条里 A、C 的格子翻成正向（给出测到的手数 / 费用）；B 的格子留着 |
| 破坏 706（「部分额度不再报错」）锚在 `if q > 0 && q < vol {`；707（「读法分岔不再报错」）锚在 `if t != lots {`；708 锚在 `q.Explicit += todayLots` | `tools/breakcheck/breaks.json` | 706 的前提（部分额度报错）不在了 ⇒ 退休；707 / 708 守的是 B，改锚点不改机制 |
| `commission` 对拆成今 / 昨两段的一笔裸平，**每段各自**算结算口径（`HalfUpToCent(按额部分)`）再相加；注释写明「放开部分额度时要改成按笔取整」（评审 20260921） | `trade.go` `commission` 的 `charge` 闭包与其上方注释 | 放开 A 就会走到拆两段：结算取整要么改按笔、要么在歧义格报错 —— 决策点 1 |
| 按额收费、平今 / 平昨两档不同的品种在大商所上存在（例 `DCE.lh2701` 平昨 0.0002 / 平今 0.0004）；郑商所按额的品种（SA / PX / SH）三档同费率 | `testdata/refdata/ctp-commission-rates-20260915.txt`（声明） | 决策点 1 的歧义在真实品种上走得到；A 的实验品种（m / MA）只有按手费，**量不到**这一格 |
| 挂单冻结（`FreezeOf`）与恢复时核冻结（`checkOrderFreeze` 遍历 k = 0…手数）都经 `commission` / `undatedCap` | `submit.go`、`state.go` | 放开之后冻结额按「拆」的读法算；恢复时的遍历已覆盖 0…手数，不用改 |
| 额度计数的存档字段不变（`Opened` / `Charged` / `Explicit`） | `state.go` `QuotaState` | **不抬 `StateFormat`**；导出面不变 |

##### 形状

- `todayTierLots(inst, vol, own, opp)`：读法只剩两种 —— 「本方向、显式平今扣额度」`own.Opened − own.Charged` 与「本方向、显式平今不扣额度」`own.Opened − (own.Charged − own.Explicit)`；
  每种读法的平今档手数 = `min(vol, max(额度, 0))`；两种给出不同手数 ⇒ 照旧报错（B，文字点名 §13 #25）。`opp` 参数删掉（额度分方向，反方向的计数不再参与）
- `commission` / `chargeUndated`：平今档 `t` 手、平昨档 `vol − t` 手，照现在的两段收费；**结算口径的取整见决策点 1**
- 文档与注释：`quota.go` 的 godoc、`trade.go` 的三处注释 —— 「这种拆法走不到」、⚠️ **「若将来放开部分额度……要改成按笔取整（评审 20260921）」**（本批之后最直接过期、而且会误导实现的一句：改成指向本节决策点 1，评审 20260922 条件 2）、「三处没实测的外推在那里报错不猜」—— §15 决策点 2 与验收里的对应句、roadmap / silent-risks / §13 #23 行里「三条外推报错」的现行说法 —— 逐条改成「A、C 已测、B 照旧报错」

##### 决策点

**1. 一笔裸平拆成今 / 昨两段、且两段都有按额部分时，结算口径按笔取整还是分段取整** —— ⚠️ 没有观测

    按笔取整   结算费 = HalfUpToCent(两段按额部分之和) + 两段按手部分之和   —— §13 #5 的「每笔重算」字面上是按笔（结算单上一笔成交一个 Fee）
    分段取整   结算费 = Σ HalfUpToCent(每段按额部分) + 按手部分          —— 现状；柜台若在内部按档分段算，就是这一种

两段的按额部分都带不到一分的零头时，两种写法**才可能**不同，但不一定不同（例：lh 两手、额度 1，有的价同值、有的价差一分）。A 的实验品种只有按手费，分不开。

- ✅ **定：分岔时报错不猜**（评审 20260922 同意；评审撤回 20260921「放开时改按笔」的建议 —— 那是按「结算单上一笔一个 Fee」的字面读法推的，柜台内部分不分档从来没量过）。另排一个事前登记的实验（`DCE.lh2701` 这类品种，一笔 2 手裸平、额度 1；单笔 2 手要临时调 `PROBE_MAX_VOLUME`，问使用者）
- ⚠️ **报错条件按值定义，不按形状**（评审 20260922 条件 1）：两种写法都算出来 —— 按笔 `HalfUpToCent(Σ按额) + Σ按手`、分段 `Σ HalfUpToCent(按额) + Σ按手` —— **不同才报错，相同照收**。与 `todayTierLots`「读法给出不同手数才报错」同一口径。
  初稿写的「只在两段都有非零按额部分这一格报」会误报（一段按额正好整分时两种同值），也漏掉「按额同价、每手不同」这种费率：那时 `tiersDiffer` 为真、照样拆两段、两段都带按额零头，两种写法会不同 —— 声明费率里今天没有这种品种，但门面接受调用方给的任意费率
- 备选（不选）：直接按笔取整。代价是一个没量过的读法静默进结算
- 盘中口径（`FeeRounding`）同一个问题、同一个判法：两个预设都**不填** `FeeRounding`（§13 #5，调用方自己选）；调用方选「不取整」时两种写法恒同值、不会报；选截断到分 / 四舍五入到分时才可能走到 —— 分岔时一起报（盘中费本来就由调用方选口径，分岔报错与「预设不替调用方选」同一原则）
- 选报错不改导出面、不需要使用者破例 ⇒ 不是使用者的决策点

##### 验收（先红后绿）

- **CTP 对拍（新增）**：从门面重放交易日 20260923 的 A、C 序列（`testdata/ctp/` 下 `ctp-slices-20260923-7.json` 到 `-20.json`：C 两所、A 两所，种子交易日 20260922 开 1 手 → Settle → 20260923 昨 1），逐笔手续费等于柜台增量：A 的 ③ 0.3 / 8、C 的 ③ 0.2 / 2。⚠️ 实现之前先跑一次看它红：A 在本库报「介于 0 与平仓量之间」，C 报「额度分不分方向」
- `TestTodayTierLots`：A、C 的格子翻成正向（2 手额度 1 ⇒ 1；反方向开过、本方向额度 0 ⇒ 0）；新增「2 手额度 1、B 两种读法同值」照收、「B 两种读法不同」照旧报错
- `TestQuotaRefusalsLeaveNoTrace`：A、C 两格移到正向测试；B 那一格留着
- 决策点 1 三格（评审 20260922 条件 1）：
  - `lh` 类费率（按额、两档不同），挑一个**两种写法不同**的价，2 手额度 1 ⇒ 报错且状态不动 —— `Submit` 与 `Place` 都要（`FreezeOf` 也经 `commission`），`ApplyTrade` 同样
  - `lh` 类费率，挑一个**两种写法同值**的价 ⇒ 照收
  - 合成费率「按额同价、每手不同」，两种写法不同 ⇒ 报错
- CTP 对拍的种子按真实时序走：交易日 20260922 开 1 手 → `Settle` → 20260923 昨 1。⚠️ C 的 `m2705` 种子那天实际开过三笔、平过一笔 2 手（F11 外推种子插曲），重放只取「昨 1」这个结果 —— 测试注释写明种子是**按结果重建**的，不是逐笔重放
- 破坏：706 退休（前提不在了，进 retiredBreakNums 写理由）；707 / 708 改锚点不改机制；新增「A 退回报错」「C 退回两个方向合计」「决策点 1 不报错」三条 —— 最后一条锚在**两种写法的比较**上

##### ⚠️ 风险与盲区

- A、C 各只有两所各一个样本（CTP 单柜台），不进 `rules_measured`；广期所等其余 NoUseHistory 交易所照旧整笔报错（`undatedCloseMeasuredOn` 不扩）
- A 只量到「平 2 手、额度 1」一格；「平 3 手、额度 1 / 2」等是同一公式 min 的外推 —— 公式由 (f) 本身给出，不再单独报错（与 F11 对 (f) 的采纳同一口径）
- A 的实验里柜台回报是**一条** 2 手记录；若某次柜台拆成多条逐手记录，从成交记录重放时本库逐条算，结果与按额度拆同值（修订后的登记已写）
- B 那一格照旧报错：调用方在「显式平今之后的裸平、两种读法不同」上拿不到数；silent-risks 103 未修
