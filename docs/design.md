...
cn-futures-rules §13 #5 在 CTP 上命中 (i-t)（⚠️ 20260917 那一次；20260918 的前置补测**未复现**，七个候选都没对上 —— 前置条件没满足，本节停在设计）：**盘中** `Commission` 是逐笔费的原样累加（柜台读数带 `16.855000000000004` 这类尾巴），
...
- 存档：`futsim.State` 新增 `Quotas []QuotaState{Instrument, Hedge, Direction, OpenedToday, ChargedToday}`，**`StateFormat` 抬一格**（与 F10、F9a 谁先落谁用下一个号）；存档字段的指纹守卫现在只在 `impl-f10` 分支上（F10 停着）—— F11 若先落，就把那条守卫一起带过来，不让「加字段忘了抬号」再次无人看守
- `Restore` 核：两个数非负、`chargedToday ≤ openedToday`、该方向今仓手数 ≤ `openedToday`（今仓都来自今天的开仓）；不许有存档交易日之外的计数

##### 决策点

1. **抬 `StateFormat`（抬一格）**：导出面 + 旧存档读不进来 ⇒ 摆给使用者（F9 / F10 的先例）
2. 以下三条**不是**使用者的决定，是**推得、未实测**，照实登记、排后续实验，不等它们开工：
   - **按方向记额度**：实验里只开过多头、只平过多头。「开空单算不算平多单的额度」没有观测 —— 按方向是最自然的读法
   - **显式平今也扣额度**：(f) 的字面是「已按平今档收过的手数」，显式平今按平今档收，就扣；大商所会把显式平昨改写成平仓（§13 #21 E2），显式平今在两所上改不改写没量过
   - **多手平仓按 min 拆**：实验全是一手一笔；两手裸平一笔成交时是否按 min(2, 额度) 拆两档，是公式外推
...
- 上面三条推得的外推（方向、显式平今、多手）任何一条错了，本库都会在那种形状上**静默**收错档 —— 与 silent-risks 100 同一个形状。每一条都要在设计里点名、在 silent-risks 里登记，并排一个事前登记的实验
