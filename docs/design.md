...
- 文档与注释：`quota.go` 的 godoc、`trade.go` 的两处注释（「这种拆法走不到」「三处没实测的外推在那里报错不猜」）、§15 决策点 2 与验收里的对应句、roadmap / silent-risks / §13 #23 行里「三条外推报错」的现行说法 —— 逐条改成「A、C 已测、B 照旧报错」
...
两者只在两段的按额部分都带不到一分的零头时分岔（例：lh 两手、额度 1）。A 的实验品种只有按手费，分不开。

- 倾向：**分岔时报错不猜**（只在「两段都有非零按额部分」这一格报；只有一段有按额部分时两种写法同值，照收），另排一个事前登记的实验（`DCE.lh2701` 这类品种，一笔 2 手裸平、额度 1；单笔 2 手要临时调 `PROBE_MAX_VOLUME`，问使用者）
- 备选：直接按笔取整（评审 20260921 当时的建议，「每笔重算」的字面读法）。代价是一个没量过的读法静默进结算
- 盘中口径（`FeeRounding`）同一个问题：`FeeRounding = 不取整` 时两段相加与整笔同值；截断 / 四舍五入到分时也会分岔 ⇒ 同一格一起报（盘中费本来就由调用方选口径，分岔时报错与「预设不替调用方选」同一原则）
- 选报错不改导出面、不需要使用者破例 ⇒ 不是使用者的决策点；评审同意即可
...
- 决策点 1 那一格：`lh` 类（按额、两档不同）2 手额度 1 ⇒ 报错且状态不动；只有一段按额（或两档同价）照收
- 破坏：706 退休（前提不在了，进 retiredBreakNums 写理由）；707 / 708 改锚点不改机制；新增「A 退回报错」「C 退回两个方向合计」「决策点 1 那一格不报错」三条
