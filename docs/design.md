...
##### 形状（实现方倾向，待评审）
    ...
    func (s *Simulator) Advance(bar Bar) ([]Filled, error)
...
##### 一根 K 线之内的顺序（约定）
    ...
    ① 守卫    bar.TradingDay == 当前交易日            不同 ⇒ 报错「先 Settle」（§4 那道守卫在门面上的落点）
              ...
              有 Calendar ⇒ Start 与 End 落在**同一个连续时段**里、所属交易日 = bar.TradingDay
...
- 倾向：**按挂单先后**。⚠️ 附带一个存档问题：F9a 之前写出的存档里没有序号 ⇒ `Restore` 读到不带序号的挂单时**报错**，不按 id 补一个顺序
  （补出来的顺序与真实挂单顺序在存档里长得一模一样）
...
##### 分期
    ...
    F9a  委托簿记挂单序号；State / Restore 带上；旧存档不带序号 ⇒ Restore 报错
    F9b  Advance 只做 ① 守卫 + ④ 计价（不撮合）—— 先把「换交易日没结算」这道守卫落到一个入口上
...
- **守卫**：交易日不同 ⇒ 报错且状态不动；时段外、跨两个时段 ⇒ 报错；High / Low 越涨跌停 ⇒ 报错；`RestingFill` 零值 ⇒ 报错
- **原子性**：第二笔 `Fill` 失败（例如资金 / 可平量）⇒ 第一笔也不算、价格也不动
- **序号**：`State` → `Restore` 往返保留挂单先后；不带序号的存档 `Restore` 报错
...
- **决策点 3 引入的序号是新的持久化字段**：存档格式变了，F5 的往返测试要跟着扩；旧存档报错是有意的，要写进 roadmap（导出面 / 存档兼容性变更）
