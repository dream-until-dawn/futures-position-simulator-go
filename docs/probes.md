...
## 14. 拒绝优先级：一笔单同时违反两项，柜台报哪一个（自然日 2026-09-09 02:0x，交易日 20260909）

`order` 的八项校验按「拒绝优先级」排序，而那个顺序整个来自
cn-futures-rules.md §9 的一张标着「**文档**」的表 —— **一次观测都没有**。

顺序错了会怎样：柜台只回一个 `last_msg`，两边都判「拒绝」，
⚠️ **差异不会以失败的形式出现**。使用者拿到的是一个指向错误位置的拒因。

### 怎么问

在**空仓合约**上构造「同时违反两项」的单子。每一笔都必然被拒，
因此不成交、不改账户状态；手数一律 1 且仍然过安全阀。

    只违反 A         → 记下柜台原话，作为标尺
    只违反 B         → 同上
    同时违反 A 和 B  → 原话等于谁，谁就赢

⚠️ **逐字比，不认关键字。** 关键字匹配（比如找「涨停」二字）会把归因变成
「我猜柜台怎么措辞」，而措辞一改，一条错误的归因会**继续绿着**。
...
两组原话不同 ⇒ **越界优先于整数倍** —— ⚠️ 与 §9 那张文档表**相反**。
另外两对（整数倍 / 越界 各与可平量）实测与文档一致。
本库据此把 `order.CheckPriceLimit` 排到 `order.CheckPriceTick` 前面。

### ⚠️ 这次实验自己出的两个错，比结论更值得记

**一、`limit` 的构造覆盖掉了 `tick` 的构造。**
`tick` 是「在现价上加个零头」，`limit` 是「把价格整个改写」。
代码按「配对里的先后」叠加，于是 `tick + limit` 这一对里
后跑的 `limit` 把零头**抹掉了** ——

    单违规 limit    @20489.7
    组合 tick+limit @20489.7   ← 同一笔单

归因于是变成**拿一笔单和它自己比**，必然「相等」，
而实验照样跑完、照样打印出一行「与本库相反」的结论。

**二、那个覆盖值本身就是脏的。** 19514 × 1.05 = 20489.7 不是 tick 的整数倍，
所以「只越涨停」这条**标尺**从一开始就同时违反了两项。

⚠️ 两个错都不会以失败的形式出现：单子发得出去、柜台照样拒、日志照样有结论。
**结论碰巧仍然成立**（13009.3333 与 20489.7 是一组干净的 A/B），
但那是运气，不是方法。

修法是给实验补一个**零层**：叠加完之后逐项核对「这一项真的成立吗」，
对不上就整条实验报错。守卫 `TestApplyAllBuildsWhatItClaims`、
破坏用例 146（把叠加顺序倒过来，当场红在「构造出来的单子对不上」）。

### 复跑

    cd cmd/oracle
    go run . probe -env ../../.env         -specs ../../testdata/refdata/specs-20260908.json         -exp reject-priority -symbols SHFE.ag2702
