...
| 快期账户能否登录、快期模拟的实际行为 | **完全未验证**，只验了协议与文档 |
| SimNow 能否注册、7×24 环境是否可用 | **完全未验证** |
| 快期模拟与真实 CTP 柜台的口径差 | **完全未测量**，这是本项目最大的一条待测量风险 |
...
并开始 [roadmap.md](./roadmap.md) 里的**六条判别实验**。
