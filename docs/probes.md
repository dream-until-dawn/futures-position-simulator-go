...
| **持仓侧**的冻结手数 `volume_*_frozen_*` | ⚠️ **几乎零观测**：375 个持仓截面里 `volume_long_frozen` 与 `_his` 各非零过 **1 次**（下午那笔裸 `CLOSE` 挂着时拍到的），另外**四个一次都没有**。⚠️ 与上一行**不是一回事**：那是账户侧的金额，这是持仓侧的手数。`position-frozen` 实验（挂→采→撤）已写好，待跑 |
