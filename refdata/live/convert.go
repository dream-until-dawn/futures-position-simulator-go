...
func fromFloat(f float64, field, who string) (decimal.Decimal, error) {
	...
	// ⚠️ **这句往返校验对任何有限 float64 都不会打响**，而且是必然的：
	//
	//	NewFromFloat  → newFromFloat(value, Float64bits(value), &float64info)
	//	                ⇒ Go 的**最短往返**十进制（唯一确定该 float 的最短字符串）
	//	Float64()     → d.Rat().Float64()
	//	                ⇒ 精确有理数 → **正确舍入**回最近的 float64
	//
	// 最短往返表示按定义唯一确定 f ⇒ 那个有理数必落在 f 的舍入区间内
	// ⇒ 正确舍入必然回到 f。（评审方 20260910 给出的结构性证明，我复核过源码。）
	//
	// ⚠️ 这一段此前写的是「它防的是哪天上游开始给高精度小数」——
	// **那是一句关于能力的假陈述**：它站在它要防的那一步的**下游**。
	// 上游的高精度在 `json.Unmarshal` 解进 float64 的那一刻就没了，
	// 到这里拿到的已经是一个自洽的 float64。
	//
	// 实测那个洞的确切形状（判据不是位数，是**最短形式**）：
	//
	//	上游文本                     json→float64→decimal
	//	0.1 / 0.005 / 0.01 / 1 / 10  原样            ✅ 所有现实的 tick 都无损
	//	0.10000000000000001          0.1             ❌ 17 位，但不是最短形式
	//	1.0000000000000002           原样            ✅ 也是 17 位，float64 分得开
	//	1.00000000000000021          1.0000000000000002 ❌
	//
	// ⇒ 只有上游发来一个**非最短形式**的十进制才会丢，而那本身就是异常数据。
	// ⚠️ 但丢了之后**没有任何东西看得见** —— 交回来的是一个干干净净的最短形式。
	//
	// ⇒ **欠着的动作**：`PriceTick` / `VolumeMultiple` 改成 `json.Number`，
	// 从**文本**建 decimal。那时这句校验既打得响，也终于对准了它要防的那件事。
	// 评审方判它为**建议不是必修**（不影响本批正确性），我同意；
	// 而在做掉它之前，这一段必须如实说明自己不具备那个能力 —— 现在这样。
	//
	// 本句留着：它是 `NewFromFloat` 行为的**绊线**。哪天依赖库改了实现，
	// TestFromFloatGuardCannotFireOnFloat64 会红，而那时上面这段说明也就该重写。
