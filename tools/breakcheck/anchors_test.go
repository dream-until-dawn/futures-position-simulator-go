...
//
// 锚点**还在、但整行被挪进了别的作用域** —— 次数仍是 1，而 new 里引用的变量在新位置不存在，
// 破坏期间编译失败（全量里判「红错了理由」）。20260915 F3 把 chargeUndated 的调用挪进 commission() 时，
// 522 的 new 引用的 `res` 就这样出了作用域。⇒ 挪动锚点所在的**行**（不只是改它的文本）之后，单跑锚在这些行上的破坏。
//
// ⚠️ 给本条写破坏时的一个坑：破坏替换掉的正是**它自己的锚点**，于是本条在破坏期间
// 必然点名那条破坏自己。⇒ 期望红的破坏，want 要点名**别的**那条（见 432）；
// 演示盲区的破坏只能插入、不能改锚点（见 433 的 why，第一版就栽在这）。
