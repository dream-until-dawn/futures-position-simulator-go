...
// # 扣的是**净额**，不是逐持仓
//
// 「只计浮亏」扣 max(账户持仓盈亏, 0)，不是 Σ max(各持仓盈亏, 0)。两者只在账上**有赚有赔**时分得开。
// 支撑它的只有**一份**截面（评审 20260915 扫出）：ctp-status-20260914-2，ag2701 +150、ag2702 −255，
// 账户 −105，柜台额外扣 0 ⇒ 净额 ✅、逐持仓（150）❌。
// ⚠️ 两条腿**同品种** ⇒「按账户净额」与「按品种净额」在它上面**分不开**；要分开得有跨品种一赚一赔的截面。
// 守卫：TestProductionAccountAvailableAgainstCTP 要求至少一份有赚有赔的截面。
//
