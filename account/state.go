...
type State struct {
	...
	// SettleCommission 是结算时计入结存的手续费（逐笔按结算口径处理后的和），≤ Commission。
	// ⚠️ 丢了它，盘中存档 → 恢复 → 结算会按「整笔都不计」算；它是 StateFormat 2 新增的那一项。
...
func (a *Account) State() State {
	return State{
		...
		CloseProfit: a.closeProfit, Commission: a.commission, SettleCommission: a.settleCommission, PositionProfit: a.positionProfit,
...
func Restore(st State) (*Account, error) {
	...
	a.settleCommission = st.SettleCommission
