...
const (
	...
	//
	// ⚠️ 它是 20260916 才有观测的：对应 `order.CheckSession`。快期那侧根本不查时段（kq_facts 48），
	// 于是这一项此前在柜台上一次都没量过。盘中休息里一笔**除时段外完全合法**的委托被拒，
	// 才把这个码逼出来（`oracle ctp-priority` 的对照组）。
	//
	// ⚠️ 语料里有**盘中休息**（11:32）与**收盘之后**（15:05）两轮，五所都是同一个码。
	// **节假日**的码没有观测，不从这一条推。（此前这里写「收盘之后没有观测」，与下面码表注释矛盾 —— 那是 11:32 那一轮时写的，评审 20260916 夜 nit 2。）
