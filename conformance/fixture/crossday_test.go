...
func TestCrossDayConformance(t *testing.T) {
	...
	// ⚠️ 类 C 是个**可证伪的预测**：今晚 21:00 行情滚到新交易日之后，
	// pre_settlement 应当变成 3163、margin 变成 6642.3，这一类应当**消失**。
	// 到那时本条会以「类 C 一次都没出现」的形式红 —— 那是好消息，
	// 而好消息同样需要有人被通知到。
	t.Log("⚠️ 类 C 是可证伪的预测：21:00 行情滚了之后它应当**消失**，" +
		"届时本条会以「类 C 一次都没出现」报红 —— 那是好消息，但清单要跟着改")
