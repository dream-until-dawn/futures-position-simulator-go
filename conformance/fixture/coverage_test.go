...
func TestFrozenFieldsAreTheThinnestEvidence(t *testing.T) {
	...
	// ⚠️ 零也要真的被数成零。
	//
	// 破坏 79（「扫描把零也算成非零观测」）在本条重写之后一度**变绿**：
	// 原判据是 len(never) > 0 才对，于是把零错算成非零会让 never 变空 —— 恰好通过。
	// 新判据要求 never == 0，而错算同样让它为 0 —— **又恰好通过**。
	//
	// 两个方向相反的判据被同一个错误一起骗过，说明它们查的都不是这件事。
	// 冻结字段的绝大多数观测**本来就是零**（没挂单的时候），
	// 所以「一个零都没数到」本身就是错的。
	totalZero := 0
	...
	if totalZero < 100 {
		t.Errorf("⚠️ 六个冻结字段一共只数到 %d 个零观测 —— "+
			"绝大多数截面上没有挂单，冻结字段本来就该是零。"+
			"这么少说明**零被算成了非零**，而那会让上下两条判据一起失效", totalZero)
	}
	if len(never) > 0 {
