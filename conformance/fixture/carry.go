...
//
// 对拍测试走它（F6b，design.md「门面的形状」§10）：记账链条只在门面里有一份。
// Carry / Reconstruct 暂留给 `cmd/oracle -carry`（那边的规格没有手续费率，门面记账要它），
// 两者由 TestReconstructMatchesFacade 逐片钉成等价。
//
// # 比 Carry 多要的三样，缺了报错
//
...
// 快期上消耗顺序结构性测不出（kq_facts 40），那道检查在快期夹具上一直在答「分不开」；它仍在 Reconstruct 里。
