...
// 它不是「可以接受这么多」，是「已经有这么多，别再多」。
//
// ⚠️ **注释里不复述这个数。** 上一版写着「已经有 69 份」而常量已经是 72：
// 一次 str.replace 没匹配上，改动静默落空，于是**棘轮最显眼的那一行
// 记着抬高之前的数** —— 而棘轮存在的全部意义，就是让「抬高它」
// 成为一个显眼的、有记录的动作。
//
// > 「已经有这么多，别再多」不写数字，就永远不会过期。
//
// ⚠️ 抬高要留记录，但记的是**理由**不是数字：
// 20260909 抬过一次 —— reject-tradable 那条实验同一晚跑了四轮，
// 前三轮在 dump 补订行情（fillQuotes）落地**之前**，因此缺行情；
// 留档不重制，它们记的是那几个时刻的账户。
// ⚠️ 一个**每次红了就往上抬**的棘轮等于没有棘轮，所以抬一次就要写一次理由。
