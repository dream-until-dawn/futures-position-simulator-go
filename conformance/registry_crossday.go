...
func CrossDay() Registry {
	...
	whyQ := "⚠️ **样本区分不了**：这个字段在**柜台那一侧**一次都没有取到过非零值。" +
		"⚠️ 20260909 更正措辞：原写「两边都是 0」，而全量对拍接上结转路径之后" +
		"**本库这一侧不再是 0**（如 open_cost_long_his 本库算出 94730 而柜台给 0，" +
		"_today 侧则是本库 0 而柜台给 \"-\"）。" +
		"⚠️ 但归类**仍然是 `?`**，而这正是最容易搞错的一步：" +
		"「两边不一样」只排除了「柜台填对了」，**没有**把" +
		"「柜台不填」与「柜台按另一个定义填、而那个定义下就是 0」分开 —— " +
		"要分开得有一次柜台把它填成非零的观测。" +
		"**差异被解释得通，不等于机制被认出来了。**"
