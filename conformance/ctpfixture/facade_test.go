...
func TestMeasuredTickRoundingAgainstCTPQuotes(t *testing.T) {
	...
	ratioPending := map[string]string{
		...
		"lh": "DCE.lh2701：F13 决策点 1 的取整实验（ctp-quota-ext -case rounding，交易日 20260924）截面顺带落了它的行情。⚠️ 比例在 CTP 上**没有来源可补**：合约查询（CThostFtdcInstrumentField）里没有涨跌幅比例这一项，§12 的比例本来就是反解来的；快期那一侧的两环（见上）同样没补。⚠️ 这批行情本身与 §13 #26 一致（11850 × 6% 往里收 = 12560 / 11140），但那是**判 #26** 用的，不拿来当本条的比例来源",
	}
