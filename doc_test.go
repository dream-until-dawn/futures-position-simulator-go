...
var goneTests = map[string]string{
	"TestSessionVersusClosablePinnedPending22": "20260916 夜改名为 TestSessionVersusClosableFollowsCTP。" +
		"⚠️ 它原本钉的是 §13 #22 **裁决前**的行为（午休 / 收盘后空仓平昨本库报时段、CTP 报可平量），名字里的 Pending22 说的就是「等裁决」；" +
		"裁决跟 CTP 后它按设计红在那两行，改成了钉 CTP 的答案。roadmap 与 silent-risks 98 里提到旧名的是**当时**的记录 —— " +
		"改写成新名会把「先钉住分岔、重排时它会红」这个做法本身抹掉",
...
var retiredBreakNums = map[string]string{
	"706": "20260922 F13 退休。它破坏的是「多手裸平、0 < 额度 < 平仓量时不再报错（按某种拆法静默收）」；F13 按交易日 20260923 两所的事前登记实验把这一格放开成按额度拆，" +
		"「部分额度就报错」这道检查本身不在了。新机制（多手退回全今）由 760 / 763 守",
	"694": "20260922 F12 退休。它破坏的是「CTP 侧已登记分歧（knownDivergence）的柜台钉值写错」；F12 把那两份样本转成正向断言、删了 knownDivergence 的钉值，前提不在了。" +
		"新机制（判别力对照组用成往里收本身）另开 743 —— 同号换机制会让评审按编号逐条比判定的做法静默失效（评审 20260922）",
	"697": "20260922 F12 退休。它破坏的是「快期侧已登记分歧（kqKnownDivergence）的柜台钉值写错」；F12 把往里收加进候选、删了那份钉值，前提不在了。" +
		"新机制（判别样本登记成四舍五入）另开 744，理由同 694",
	...
	"4": "⚠️ 同上：前缀 4 当时被 11 条共用（新号 359）。" +
		...
		"改写成一个唯一编号会让那句话变成废话",
	// ⚠️ 422 是**第一条因为「那个错误变成不可能了」而退役**的破坏，
	// 与 1 / 4 那两条（重编号的历史）性质不同，所以理由要写全。
	"422": "⚠️ 它原先钉「`len(msgHas) != len(want)` 那道自检自己没被测」。" +
		"20260912 评审判它「机械上与 418/419 同解，而价值明显更低 —— " +
		"没被测的是**测试表格自身写没写齐**，隔了一层」，并明说不推着做。" +
		"⇒ 而我选了**第三个选项**：把那两个平行切片**绑成一个结构体**，" +
		"于是「两边不一样长」在语法上说不出口 —— **那道自检连同这条欠债一起消失了**。" +
		"⚠️ 留在这张表里是因为：**退役与遗忘在清单上长得一模一样**，" +
		"而讲这件事的那几段文字（state.md / silent-risks）说的正是「它为什么不再需要」。" +
		"⚠️⚠️ 而这一条的存在本身是个提醒：**把错误变成不可能，胜过多一条守卫去防它** —— " +
		"于是「破坏总数」这个计数会**下降**，而那是好事，不是回归",
	"662": "20260916 夜 §13 #22 裁决跟 CTP 后退休。它破坏的是「门面不再拒在时段 —— 时段 × 可平量悄悄换成了 CTP 那一侧的答案，而 #22 还没裁决」，" +
		"而裁决之后 CTP 那一侧就是本库的答案，前提反过来了；且可平量已排在时段之前，去掉时段拒单不再改变空仓平昨的结果 —— 它会永远绿。" +
		"改形状违反「改指一条破坏只该动锚点」，只能退休；「顺序挪回快期那一侧」由新号 665 守",
