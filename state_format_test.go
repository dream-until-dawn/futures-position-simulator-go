...
// 格式 1 是回填的：本表 20260918（F10）才建，按 b11c600（F10 之前的 main）的字段集合算 = 格式 2 去掉 Account.SettleCommission。
// ⚠️ F5 到 b11c600 之间格式 1 的字段有没有变过而没抬号，本表回答不了 —— 这张表防的是今后。
var stateFingerprints = map[int]string{
	1: "a2b10ae2455cd8891e10313f00448bba",
	2: "b0b4e4089e2e7271ef3e77beae41c6bf",
}
...
func TestStateFormatPinsFieldSet(t *testing.T) {
	...
	if !strings.Contains(strings.Join(fields, "\n"), "State.Account.SettleCommission ") {
		t.Fatal("⚠️ 字段表里没有 Account.SettleCommission —— 遍历没走进嵌套结构")
	}
...
// TestRestoreRefusesOlderFormat：格式 1 的存档（F10 之前，没有 SettleCommission）在版本号那一步报错，不迁移。
